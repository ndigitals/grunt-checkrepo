...
var git = {
  ...
  isTagged: function () {
    return !!exec("git tag --points-at HEAD")
      .stdout.split("\n")
      ...
      var checks = {
          ...
          methods.forEach(function (method) {
            if (method === "valid") {
              ...
              if (semver[method](highestTag, config[method])) {
                grunt.verbose.ok(
                  'Highest tag "' +
                    String(highestTag).cyan +
                    '" is ' +
                    method +
                    ' "' +
                    String(config[method]).cyan +
                    '".'
                );
              } else {
                failed(
        ...
        tagged: function (flag) {
          if (git.isTagged() === flag) {
            grunt.verbose.ok(
              "Current commit is" + (flag ? "" : " not") + " tagged."
            );
            return true;
          } else {
        ...
        clean: function (flag) {
          if (git.isClean() === flag) {
            grunt.verbose.ok(
              "There are" + (flag ? " no" : "") + " unstaged changes."
            );
            return true;
          } else {
